...
class PageContainerState extends State<PageContainer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _slideAnimation = Tween(begin: Offset(1.0, 0.0), end: Offset.zero).animate(_controller);
    _controller.forward(from: 0.0);
  }
  ...
  Widget build(BuildContext context) {
    return FadeTransition(
      ...
      child: SlideTransition(
          ...
          child: (widget.children.length == 0 || _currentPageIndex >= widget.children.length)
              ? Container()
              : widget.children[_currentPageIndex],
      ),
    );
  }
  ...
  void changePage(int index) {
    if (index < 0 || index >= widget.children.length) {
      throw Exception('切换页面出错, 需要切换到$index页，实际长度只有${widget.children.length}}');
    }
